...
"env N (Ident n) = (if Ident n = Ident ''Sta.x'' then
  boolType
else
  anyType
)"|
"env N (Para n i) = (if Para n i = Para ''Sta.n'' i \<and> (i \<le> N) then
  enumType
else
  anyType
)"|
...
lemma env_simp : 
             "[|i <= N|] 
                 ==> env N (Para ''Sta.n'' i) = enumType"
                 

" 
                  env N (Ident ''Sta.x'') = boolType"
...
definition initSpec0 :: "nat \<Rightarrow> formula" where [simp]:
 "initSpec0 N \<equiv> (\<forall>\<^sub>fi. IVar (Para ''Sta.n'' i) =\<^sub>f Const I) N"
...
definition initSpec1 :: "formula" where [simp]:
 "initSpec1 \<equiv> IVar (Ident ''Sta.x'') =\<^sub>f Const true"
...
definition Try :: "nat \<Rightarrow> rule" where
 "Try i \<equiv> 
   IVar (Para ''Sta.n'' i) =\<^sub>f Const I
  \<triangleright>
   assign (Para ''Sta.n'' i, Const T)"
...
definition Crit :: "nat \<Rightarrow> rule" where
 "Crit i \<equiv> 
   IVar (Para ''Sta.n'' i) =\<^sub>f Const T \<and>\<^sub>f
   IVar (Ident ''Sta.x'') =\<^sub>f Const true
  \<triangleright>
   assign (Para ''Sta.n'' i, Const C) ||
   assign (Ident ''Sta.x'', Const false)"
...
definition Exit :: "nat \<Rightarrow> rule" where
 "Exit i \<equiv> 
   IVar (Para ''Sta.n'' i) =\<^sub>f Const C
  \<triangleright>
   assign (Para ''Sta.n'' i, Const E)"
...
definition Idle :: "nat \<Rightarrow> rule" where
 "Idle i \<equiv> 
   IVar (Para ''Sta.n'' i) =\<^sub>f Const E
  \<triangleright>
   assign (Para ''Sta.n'' i, Const I) ||
   assign (Ident ''Sta.x'', Const true)"
...
definition MutualExclusion :: "nat \<Rightarrow> nat \<Rightarrow> nat \<Rightarrow> formula" where
 "MutualExclusion N i j \<equiv> \<not>\<^sub>f Const (index i) =\<^sub>f Const (index j) \<longrightarrow>\<^sub>f
  \<not>\<^sub>f (IVar (Para ''Sta.n'' i) =\<^sub>f Const E \<and>\<^sub>f
  IVar (Para ''Sta.n'' j) =\<^sub>f Const E)"
...
definition ABS_Idle :: "nat \<Rightarrow> rule" where
 "ABS_Idle N \<equiv> 
   (\<forall>\<^sub>fj. \<not>\<^sub>f IVar (Para ''Sta.n'' j) =\<^sub>f Const C \<and>\<^sub>f
   \<not>\<^sub>f IVar (Para ''Sta.n'' j) =\<^sub>f Const E) N
  \<triangleright>
   assign (Ident ''Sta.x'', Const true)"
...
definition Lemma_1 :: "nat \<Rightarrow> nat \<Rightarrow> nat \<Rightarrow> formula" where
 "Lemma_1 N p i \<equiv> IVar (Para ''Sta.n'' i) =\<^sub>f Const E \<longrightarrow>\<^sub>f
  forallFormExcl (\<lambda>j. \<not>\<^sub>f IVar (Para ''Sta.n'' j) =\<^sub>f Const C \<and>\<^sub>f
  \<not>\<^sub>f IVar (Para ''Sta.n'' j) =\<^sub>f Const E) i N"
...
lemma symMutualExclusion : 
                  " 
                  symParamForm2 N (MutualExclusion N)"
                 unfolding MutualExclusion_def  apply(auto      )    
...
definition Idle_ref :: "nat \<Rightarrow> nat \<Rightarrow> rule" where
 "Idle_ref N i \<equiv> 
   IVar (Para ''Sta.n'' i) =\<^sub>f Const E \<and>\<^sub>f
   forallFormExcl (\<lambda>j. \<not>\<^sub>f IVar (Para ''Sta.n'' j) =\<^sub>f Const C \<and>\<^sub>f
   \<not>\<^sub>f IVar (Para ''Sta.n'' j) =\<^sub>f Const E) i N
  \<triangleright>
   assign (Para ''Sta.n'' i, Const I) ||
...
definition Lemma_1' :: "nat \<Rightarrow> nat \<Rightarrow> nat \<Rightarrow> formula" where
 "Lemma_1' N i j \<equiv> \<not>\<^sub>f Const (index i) =\<^sub>f Const (index j) \<and>\<^sub>f
  IVar (Para ''Sta.n'' i) =\<^sub>f Const E \<longrightarrow>\<^sub>f
  \<not>\<^sub>f IVar (Para ''Sta.n'' j) =\<^sub>f Const C \<and>\<^sub>f
  \<not>\<^sub>f IVar (Para ''Sta.n'' j) =\<^sub>f Const E"
